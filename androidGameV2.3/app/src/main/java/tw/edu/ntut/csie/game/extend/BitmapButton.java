...
public class BitmapButton implements GameObject, PointerEventHandler {
    ...
    public boolean pointerReleased(List<Pointer> pointers) {
        ...
        if (pointers.size() == 1 && _visible) {
            if (pointers.get(0).getType() == Pointer.LEFT_MOUSE_BUTTON && contains(pointers.get(0))) {
    ...
    public boolean pointerMultiReleased(List<Pointer> pointers) {
        _isNone = false;
        if (pointers.size() <= 3 && _pressed && _visible) {
            for (int i = 0; i < pointers.size(); i++) {
                Pointer pointer = pointers.get(i);
                if (pointer.getType() == Pointer.LEFT_MOUSE_BUTTON && !contains(pointer)) {_isNone = true;}
            }
            if (_isNone) {
                _pressed = false;
                notifyButtonEventHandlers();
     ...
     */
    private boolean contains(Pointer pointer) {return contains(pointer.getX(), pointer.getY());
    }
